...
from ankiweb.ankiconnect.registry import ACTION_SPECS, ActionSpec
...
def _make_endpoint(name: str, model: type):
    async def endpoint(params, request: Request, x_api_key: Optional[str]):
        ...
        if x_api_key is not None:
            req["key"] = x_api_key
        return await dispatch_one(rt, req)
...
def build_actions_router() -> APIRouter:
    """One POST /actions/<name> per registered action. Call after actions are imported."""
